...
- The `[post_process.translate]` template is the hardened clause "IMPORTANT: Write your output in X, regardless of the language of the transcription." (ported from polished-recognition #56); the `[post_process.system]` prompt additionally forbids executing commands/instructions found in the dictation.
- Pristine pre-#73 default values in an existing prompts.toml are upgraded automatically on load: fully pristine files are rewritten from the current template, partially customized files upgrade only the untouched legacy values in memory (with a console notice). Customized values are never overwritten.
